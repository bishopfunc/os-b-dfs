...
func (w *ClientWrapper) healthCheck() {
	...
	resp, err := w.client.FilePathWalk(w.ctx, &pb.FilePathWalkRequest{Filepath: path})

	// clientFilesとserverFilesをセットに変換
	clientFileSet := make(map[string]struct{})
	for _, file := range clientFiles {
		clientFileSet[file] = struct{}{}
	}

	serverFileSet := make(map[string]struct{})
	for _, file := range resp.Filenames {
		serverFileSet[file] = struct{}{}
	}

	// セットを比較
	if reflect.DeepEqual(clientFileSet, serverFileSet) {
		fmt.Println("health check: ok")
	} else {
		fmt.Println("health check: ng")
		fmt.Println("client files: ", clientFileSet)
		fmt.Println("server files: ", serverFileSet)
	}
}
